...
We can see that the frequency of difference of life expectancy between men and women shows a Poisson shape, bounded at 0, discrete values and a little bit skewed to the left. Hence, will use local Poisson regression to estimate this variable
...
pred <- predict(aux.glm, 
                ...
                type="response")
lines(aux$eval.points,pred,col=2)
legend('topright', c("h.cv.sm.poisson", "sm.regression", "glm"), col = c('black', 'red', 'blue'), lty = c(1, 1, 1))

The h valuse are similar: `r h.list.ll$h.cv`,`r h.list.ps$h.cv`
...
In this case, we can see that the model fitted with the regression function of the sm library could not adjust that well this time. This could be due to the fact that it is a non parametric model and no distribution is assumed, therefore, .....
