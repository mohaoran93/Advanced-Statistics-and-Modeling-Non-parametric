...
```
\
From the plot of the first ancova using $h1=$ `r h1` we can see that the 4 curves behave differently, however, the group 1 and 3 have similar curves#TODO: {Please check, I don't think 1 and 3 have similar curves, please check.}. If we check the p-value we see that this is close but lower than 0.05, which means the rejection of null hypothesis and indicating that the regression functions are not equal.
