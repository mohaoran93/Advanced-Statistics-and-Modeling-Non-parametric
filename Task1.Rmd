...
tg = seq(min(Yr), max(Yr), length=709) # why length=51 before? 
for (h in hs) {
...
lpr_visual(x=Yr, y=lgWeight, h=h, q=degree, tg=c(20,40,60,80),
           xlab="Yr", ylab="lgWeight",
           main=paste('Degree local pol.: q =', paste(degree, paste(", h = ", h))), type.kernel="epan")
lpr <- locpolreg(x=Yr, y=lgWeight, h=h, q=degree, r=0, type.kernel="epan") # TODO what is the main=paste("r=1, q=",paste(degree, paste(", h=",h))) for? #TODO I think the order should be 0 in our case, do we care first derivate?
```
...
# step 3 Fit a nonparametric regression to data (Yr,z_i) and 
# call the estimated function qˆ(x).
lpr2 <-locpolreg(x=Yr, y=z_i, h=h, q=degree, r=0, type.kernel="epan")# main=paste("r=1, q=",paste(degree, paste(", h=",h)))
```
