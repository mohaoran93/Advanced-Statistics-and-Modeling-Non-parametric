...
par(mfrow=c(3,5))
hs <- c(1,1.4,4,8.4,10)
tg = seq(min(Yr), max(Yr), length=51)
for (h in hs) {
...
# TODO I do not think that t really influences the estimator.

```{r}
h <- 4
llr <- loc.lin.reg(x=Yr, y=lgWeight, h=h, tg=tg)
# TODO with any other length except 709, it will cause problem on claculating log((lgWeight - llr$mt)^2) : longer object length is not a multiple of shorter object length
```
