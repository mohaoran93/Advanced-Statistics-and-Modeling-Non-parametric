...
After fitting a linear regression we are going to try with local polynomial regression of different degress, using the scaled Epanechnikov as the kernel function.
```{r}
par(mfrow=c(2,2))
...
for (d in degrees) {
  lpr_visual(x=Yr, y=lgWeight, q=d, tg=c(20,40,60,80), # h=10
             ...
             main=paste('Degree local pol.: q = ', d, sep = ""), type.kernel="rs.epan")
}
```
...
lpr_visual(x=Yr, y=lgWeight, h=h, q=degree, tg=c(20,40,60,80),
           xlab="Yr", ylab="lgWeight",
           main=paste('Degree local pol.: q =', paste(degree, paste(", h = ", h))), type.kernel="rs.epan")
lpr <- locpolreg(x=Yr, y=lgWeight, h=h, q=degree, r=0, type.kernel="rs.epan")
```

Repeating the same proceure we used before for the local linear regression, we will transform the estimated residuals and perform the polynomial regression over $z_i$
```{r}
par(mfrow=c(2,2))
...
# step 3 Fit a nonparametric regression to data (Yr,z_i) and 
# call the estimated function qˆ(x).
lpr2 <-locpolreg(x=Yr, y=z_i, h=h, q=degree, r=0, type.kernel="rs.epan", 
...
After performing performing a local linear and polynomial regressions we can see that the results are comparable, the polynomial showing a smoother curve. Hence, we can say that for this escenario of the aircraft data both approaches are satifactory.