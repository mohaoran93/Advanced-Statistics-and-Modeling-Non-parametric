...
for (k in 1:n_folds) {
  ...
  x <- x$x
  fitted_models <- apply(t(df), 2, function(degf) lm(y ~ bs(x =x, df = degf,degree = 3))) # mark
  ...
  pred <- mapply(function(obj, degf) predict(obj, data.frame(bs(x, df = degf,degree = 3))),fitted_models, df)
